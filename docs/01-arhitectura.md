...
| **`output: "standalone"` + `outputFileTracingIncludes` pe `/api/**`**     | Build-ul standalone servește rularea self-host local; rutele server-only citesc `data/*.json` cu `fs.readFile(path.join(process.cwd(), …))` la runtime, deci glob-ul `"/api/**": ["./data/**"]` include explicit fișierele în bundle-ul fiecărei rute API — contract declarat, nu efect implicit al file-tracing-ului (un refactor spre căi dinamice sau un schimb major de tracing l-ar rupe silențios; azi tracing-ul implicit le include deja — verificat pe artefacte `.nft.json`). Paginile sunt scutite intenționat: client-only, datele vin prin React Query. |

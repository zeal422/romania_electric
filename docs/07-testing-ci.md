...
5. **`test`** — `bun test` (227 de teste unitare)
...
- **Locație**: `tests/sen/` (8 fișiere) + `tests/local-preference.test.ts` + `tests/storage.test.ts` + `tests/capture-storage.test.ts` + `tests/capture-prices.test.ts` — **12 fișiere, 227 de teste** (plus verificări pe date reale în loop).
- **Ce acoperă**:
...
- Typecheck: curat · Lint: curat · Prettier: curat · 227/227 teste · Build standalone reușit.

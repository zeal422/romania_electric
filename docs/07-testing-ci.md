...
5. **`test`** — `bun test` (140 de teste unitare)
...
- **Locație**: `tests/sen/` (4 fișiere) + `tests/local-preference.test.ts` + `tests/storage.test.ts` + `tests/capture-storage.test.ts` — **7 fișiere, 140 de teste** (plus verificări pe date reale în loop).
- **Ce acoperă**:
...
- Typecheck: curat · Lint: curat · Prettier: curat · 140/140 teste · Build standalone reușit.

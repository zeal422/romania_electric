...
5. **`test`** — `bun test` (179 de teste unitare)
...
- **Locație**: `tests/sen/` (6 fișiere) + `tests/local-preference.test.ts` + `tests/storage.test.ts` + `tests/capture-storage.test.ts` — **9 fișiere, 179 de teste** (plus verificări pe date reale în loop).
- **Ce acoperă**:
...
- Typecheck: curat · Lint: curat · Prettier: curat · 179/179 teste · Build standalone reușit.

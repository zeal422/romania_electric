...
5. **`test`** — `bun test` (216 de teste unitare)
...
- **Locație**: `tests/sen/` (7 fișiere) + `tests/local-preference.test.ts` + `tests/storage.test.ts` + `tests/capture-storage.test.ts` + `tests/capture-prices.test.ts` — **12 fișiere, 216 de teste** (plus verificări pe date reale în loop).
- **Ce acoperă**:
...
| `tests/sen/costs.test.ts`               | `priceForHour` (aliniere interval N = ora N−1; zi lipsă / array scurt → undefined), `bucketHours` (1h / 10min / 24h), `computeCosts` (invariantul cost = Σ importMWh × preț orar; import/export MWh; venit; net; ore fără preț excluse + `hasPrices=false`; granularitate `10m` → MW→MWh; sold 0 → zero), `intervalStats` (medii + vârf; gol → zerouri), `formatEurMillions` (milioane EUR cu semn, non-finit → „—")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `tests/capture-prices.test.ts`          | Logica Python `--capture-prices` rulată real: `parse_prices_csv` (24 intervale, payload gol/header lipsă/non-numeric/NaN → None), `merge_prices` (zi nouă, suprascriere la prețuri diferite, indempotent la identic, record-uri invalide excluse), end-to-end cu mock server + fișier temp (`SEN_PRICES_URL_TEMPLATE`/`SEN_PRICES_OUT`/`SEN_PRICES_BACKFILL_DAYS`): scriere cu shape corect, payload gol → nimic, eșec de rețea → grațios + date păstrate, fișier corupt/non-list → restart curat, backfill → suprascrie zilele existente + ordine cronologică                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
- Typecheck: curat · Lint: curat · Prettier: curat · 216/216 teste · Build standalone reușit.

...
5. **`test`** — `bun test` (131 teste unitare)
...
- **Locație**: `tests/sen/` (4 fișiere) + `tests/local-preference.test.ts` + `tests/storage.test.ts` + `tests/capture-storage.test.ts` — **7 fișiere, 131 teste** (plus verificări pe date reale în loop).
- **Ce acoperă**:
...
- Typecheck: curat · Lint: curat · Prettier: curat · 131/131 teste · Build standalone reușit.

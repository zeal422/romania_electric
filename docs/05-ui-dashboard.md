...
| `Filters`            | `filters.tsx`              | Preset-uri de interval (`RANGE_PRESETS`: 24h/3d/7d/30d/all, relative la `endTs`), selector granularitate (opțiuni incompatibile dezactivate prin `granularitiesForPreset` din `lib/sen/types.ts`), buton export CSV                                                                                                                                                                                                                                                            |
...
| `StorageCard`        | `storage-card.tsx`         | Mini-card „Stocare": valoarea curentă ISPOZ (MW), trend față de ultima captură (încărcare/descărcare/stabil) și sparkline SVG cu seria acumulată de capturi orare — în sidebar, lângă Mixul curent. Badge-ul reflectă **proveniența** valorii (`current.source`): „live" pentru snapshot de la `/sen-filter` (proaspăt sau stale) vs „ultima captură" pentru punctul din istoric. Culoarea accent (`#A582FF`) e importată din `constants.ts` (`STORAGE_COLOR`), nu hardcodată. |

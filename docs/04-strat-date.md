...
- `customRangeToBoundaries(customRange, startTs, endTs)` → interval personalizat (calendar range) → granițe epoch clampate la datele disponibile: `from` = 00:00 al zilei de start, `to` = 23:59:59.999 al zilei de final (granițe UTC, contract fake-UTC); `null` pentru input invalid, **dată inexistentă** (ex: `2026-02-30` — parse strict `YYYY-MM-DD` + round-trip prin `Date.UTC`, pentru că `new Date()` normalizează silențios ziua peste limita lunii, iar `Number.isFinite` nu prinde asta — fix 0.3.27), `from > to` **sau range fără suprapunere cu seria** (după clamp, `from > to` → `null`; apelantul cade pe preset). Extrasă din `page.tsx` (era închisă în componentă, netestabilă) — pură, testată (fix 0.3.26).
- `toLocalDateKey(d)` → cheia calendaristică `YYYY-MM-DD` din **componentele locale** (`getFullYear/getMonth/getDate`, zero-pad). Folosită de `Filters`/`RangePicker` pentru conversia datei alese în react-day-picker (care produce miezul nopții LOCAL): `toISOString().slice(0, 10)` ar lua ziua UTC — pentru UTC+3 (România) click-ul pe „15 aug" ar deveni „2026-08-14" (off-by-one, fix 0.3.27); getters-ii locali redau mereu ziua văzută în calendar, în orice fus. Pură, testată. **Locul ei e `calendar.ts`**, NU `format.ts`: `format.ts` afișează datele sursă strict prin getters UTC (contract fake-UTC, AGENTS §4.7), în timp ce input-ul calendarului trăiește în fusul LOCAL al browser-ului — separarea modulelor ține contractul UTC-only al lui `format.ts` intact (fix 0.3.27).

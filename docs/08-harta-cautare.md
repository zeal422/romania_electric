...
| Unde e garda anti-getter-local (contractul fake-UTC)?   | [`eslint.config.mjs`](../eslint.config.mjs) → obiectul cu `files: ["src/lib/sen/**/*.ts"]` (excepție intenționată: `calendar.ts`)                                                                                                                                                                     |
| Unde e declarat tracing-ul `data/*.json` pe rutele API? | [`next.config.ts`](../next.config.ts) → `outputFileTracingIncludes` cu glob-ul `"/api/**"` (v0.3.34 — contract explicit, nu efect implicit al file-tracing-ului)                                                                                                                                      |
